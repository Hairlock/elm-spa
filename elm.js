...
Elm.Main.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var httpTask = $Http.getString("http://localhost:3005/");
   var mb = $Signal.mailbox("");
   var sendToMb = function (result) {
      return A2($Signal.send,mb.address,result);
   };
   var runTask = A2($Task.andThen,httpTask,sendToMb);
   var clockSignal = $Time.every(2 * $Time.second);
   var taskSignal = A2($Signal.map,
   $Basics.always(runTask),
   clockSignal);
   var runner = Elm.Native.Task.make(_elm).performSignal("runner",
   taskSignal);
   var view = function (message) {    return $Html.text(message);};
   var main = A2($Signal.map,view,mb.signal);
