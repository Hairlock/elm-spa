...
{-
    Available views in teh application
    NotFoundView used when no route matches the location
-}
...
{-
    Need to store the payload given by Hop
    and also the current view
-}    
...
update action model =
    case action of
        {-
            NavigateTo
            Called from our application views, asks hop
...
{-
    Create a Hop router
    Hop expects a list of routes and an action for a location
    that doesnt match
-}    
